...
export const TicketDetailPage: React.FC<TicketDetailPageProps> = ({
  ...
  const getPriorityBadge = (priority: string) => {
    switch (priority) {
      case 'urgent':
        return <span className="bg-rose-100 text-rose-700 border border-rose-300 text-[11px] font-bold px-2.5 py-0.5 rounded-full">فوری</span>;
      case 'high':
        return <span className="bg-amber-100 text-amber-700 border border-amber-300 text-[11px] font-bold px-2.5 py-0.5 rounded-full">بالا</span>;
      default:
        return <span className="bg-blue-100 text-blue-700 border border-blue-300 text-[11px] font-bold px-2.5 py-0.5 rounded-full">عادی</span>;
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'answered':
        return <span className="bg-emerald-50 text-emerald-700 border border-emerald-200 text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1"><CheckCircle2 className="w-3.5 h-3.5" /> پاسخ داده شده</span>;
      case 'in_progress':
        return <span className="bg-blue-50 text-blue-700 border border-blue-200 text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1"><Clock className="w-3.5 h-3.5" /> در حال بررسی توسط انبار</span>;
      case 'closed':
        return <span className="bg-slate-100 text-slate-600 text-xs font-bold px-3 py-1 rounded-full">بسته شده</span>;
      default:
        return <span className="bg-amber-50 text-amber-700 border border-amber-200 text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1"><Clock className="w-3.5 h-3.5" /> در انتظار پاسخ کارشناس</span>;
    }
  };
    ...
    <div className="space-y-6" id="ticket-detail-view">
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-5 sm:p-6 shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h1 className="text-base sm:text-lg font-black text-slate-900 mt-1.5">
                {ticket.title}
              </h1>
            </div>
          </div>

          <div className="text-xs text-slate-500 flex flex-col sm:items-end gap-1">
        ...
        {ticket.orderTrackingCode && (
          <div className="mt-4 pt-3 border-t border-slate-100 flex items-center gap-2 text-xs text-blue-600 ">
              ...
              if (isSystem) {
                return (
                  <div key={msg.id} className="text-center my-3">
                    <span className="text-[11px] font-bold bg-slate-100 text-slate-500 px-3 py-1 rounded-full border border-slate-200 ">
                      {msg.text}
                    </span>
                  </div>
                );
              ...
              return (
                <div 
                  ...
                  className={`rounded-2xl p-4 sm:p-5 border transition-all ${
                    isCustomer 
                      ? 'bg-blue-50/70 border-blue-200 mr-0 sm:mr-6'
                      : 'bg-emerald-50/70 border-emerald-200 ml-0 sm:ml-6'
                  }`}
                >
                  <div className="flex items-center justify-between gap-2 mb-2 pb-2 border-b border-slate-200/40 ">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        {!isCustomer && (
                          <span className="text-[10px] text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-full mr-2 font-bold">
