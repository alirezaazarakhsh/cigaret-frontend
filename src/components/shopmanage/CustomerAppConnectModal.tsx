...
export const CustomerAppConnectModal: React.FC<CustomerAppConnectModalProps> = ({
  ...
  // In-App Cart Helpers
  const handleAddToCart = (product: CigaretteProduct, unit: 'carton' | 'box' = 'carton') => {
    setPickupCart(prev => {
  ...
  const handleUpdateCartQty = (productId: string, unit: 'carton' | 'box', delta: number) => {
    setPickupCart(prev => {
      return prev.map(item => {
  ...
  const handleRemoveFromCart = (productId: string, unit: 'carton' | 'box') => {
  ...
  return (
    ...
    >
      <div 
        className={`bg-white border border-slate-200 rounded-3xl w-full p-4 sm:p-6 shadow-2xl my-auto transition-all ${
          isFullScreenPhone ? 'max-w-md' : 'max-w-6xl'
        }`}
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            >
              ...
              <span>{isFullScreenPhone ? 'نمایش پنل مدیریت' : 'تمرکز روی گوشی مشتری'}</span>
            </button>
        ...
        <div className={`mt-4 grid gap-6 ${isFullScreenPhone ? 'grid-cols-1' : 'grid-cols-1 lg:grid-cols-12'}`}>
          ...
          <div className={`${isFullScreenPhone ? 'w-full' : 'lg:col-span-6'} flex flex-col items-center justify-center`}>
            ...
            <div className="w-full max-w-[420px] bg-slate-900 rounded-[38px] p-3 shadow-2xl border-[5px] border-slate-800 text-slate-100 flex flex-col relative overflow-hidden">
              ...
              <div className="bg-slate-950 rounded-[28px] overflow-hidden flex flex-col text-slate-200 min-h-[580px] max-h-[640px] border border-slate-800/80">
                                ...
                                <div className="flex items-start justify-between gap-2">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <div>
                                      ...
                                      <span className="text-[9px] text-slate-400 block">{prod.brand} | {prod.boxesPerCarton} باکس در کارتن</span>
                                    </div>
                                  </div>
                                  <span className="px-1.5 py-0.5 rounded-md bg-emerald-950 text-emerald-400 text-[9px] font-bold">
                                    موجود انبار
                                  </span>
                                </div>
                                ...
                                <div key={`${item.product.id}_${item.unit}_${idx}`} className="bg-slate-900 border border-slate-800 p-2.5 rounded-2xl space-y-1.5">
                                  <div className="flex items-center justify-between">
                                    <div className="min-w-0">
                                      ...
                                      <span className="text-[9px] text-slate-400">واحد: {item.unit === 'carton' ? 'کارتن پلمپ' : 'باکس'}</span>
                                    </div>
                                    <button
                                  ...
                                  <div className="flex items-center justify-between pt-1 border-t border-slate-850">
                                    ...
                                    <span className="font-mono font-black text-amber-300 text-xs">
                                      {formatToman((item.unit === 'carton' ? item.product.cartonPrice : item.product.boxPrice) * item.quantity)}
                                    </span>
                                  </div>
                                </div>
                              ))}
