...
export const AccountingPosPanel: React.FC<AccountingPosPanelProps> = ({
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-4 selection:bg-indigo-600 selection:text-white" dir="rtl">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-800/80 border border-slate-700 rounded-xl pr-10 pl-4 py-3 text-sm text-white font-mono focus:border-indigo-500 focus:outline-none transition-colors"
                  required
                />
              </div>
            </div>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-indigo-600 selection:text-white" dir="rtl">
      ...
      <header className="bg-slate-900 border-b border-slate-800 sticky top-0 z-40 px-4 sm:px-8 py-3">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 text-[10px] font-black px-2 py-0.5 rounded-full">
                  صندوق آنلاین
                </span>
              </div>
          ...
          {/* Sub Navigation Tabs */}
          <div className="flex items-center gap-1.5 bg-slate-800/80 p-1 rounded-2xl border border-slate-700/60 overflow-x-auto">
            <button
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onReturnToStore}
              className="flex items-center gap-1.5 px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white text-xs font-bold rounded-xl border border-slate-700 transition-colors"
            >
              <ArrowRight className="w-4 h-4" />
      ...
      <div className="bg-slate-900/50 border-b border-slate-800/80 px-4 sm:px-8 py-4">
        <div className="max-w-7xl mx-auto grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex items-center justify-between">
            <div>
              <p className="text-[11px] text-slate-400 font-medium">ارزش کل موجودی انبار</p>
              <p className="text-base sm:text-lg font-black text-indigo-400 mt-0.5">{formatToman(totalInventoryValue)}</p>
            </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex items-center justify-between">
            <div>
              <p className="text-[11px] text-slate-400 font-medium">کل کارتن‌های موجود</p>
              <p className="text-base sm:text-lg font-black text-emerald-400 mt-0.5">{formatNumberFa(totalCartonsInStock)} کارتن</p>
            </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex items-center justify-between">
            <div>
              <p className="text-[11px] text-slate-400 font-medium">فروش ثبت شده صندوق</p>
              <p className="text-base sm:text-lg font-black text-cyan-400 mt-0.5">{formatToman(todaySalesTotal)}</p>
            </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex items-center justify-between">
            <div>
              <p className="text-[11px] text-slate-400 font-medium">کالاهای رو به اتمام (کسری)</p>
              <p className="text-base sm:text-lg font-black text-amber-400 mt-0.5">{formatNumberFa(lowStockCount)} کالا</p>
            </div>
        ...
        {activeSubTab === 'pos' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-7 space-y-4">
              ...
              <div className="bg-slate-900 border border-indigo-500/30 rounded-3xl p-5 shadow-xl relative overflow-hidden">
                <div className="flex items-center gap-2 mb-3">
                  <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-ping" />
                  <h2 className="text-sm font-black text-white">
                    اسکن بارکد خوان فروشگاهی یا جستجوی سریع
                  </h2>
                </div>

                <form onSubmit={handleBarcodeSubmit} className="relative">
                  <Barcode className="w-6 h-6 absolute right-4 top-1/2 -translate-y-1/2 text-indigo-400" />
                  <input
                    ref={barcodeInputRef}
                    type="text"
                    value={barcodeInput}
                    onChange={(e) => setBarcodeInput(e.target.value)}
                    placeholder="بارکدخوان فعال است: بارکد پاکت/باکس را بزنید یا اینتر کنید..."
                    className="w-full bg-slate-950 border-2 border-indigo-500/50 rounded-2xl pr-14 pl-28 py-3.5 text-sm sm:text-base font-bold text-white placeholder-slate-500 focus:border-indigo-400 focus:outline-none focus:ring-4 focus:ring-indigo-500/20 transition-all font-mono"
                  />
                  <button
                    type="submit"
                    className="absolute left-2 top-1/2 -translate-y-1/2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-black shadow-md transition-colors"
                  >
                    ثبت بارکد
                  </button>
                </form>
                ...
                <div className="mt-3 flex items-center justify-between text-[11px] text-slate-400">
                  <span>نمونه بارکدها: مارلبرو (7610111245012)، وینستون (4033100112348)</span>
                  <span className="text-indigo-400 font-bold">کلید میانبر اینتر ↵</span>
                </div>
              </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 mb-4">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl pr-10 pl-4 py-2.5 text-xs text-white placeholder-slate-500 focus:border-indigo-500 focus:outline-none font-medium"
                    />
                  </div>
                  ...
                  <select
                    ...
                    className="bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-xs font-bold text-slate-200 focus:border-indigo-500 focus:outline-none"
                  >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 max-h-[500px] overflow-y-auto pr-1">
                  {filteredPosProducts.map((prod) => (
                    <div
                      ...
                      className="bg-slate-950 hover:bg-slate-800/80 border border-slate-800 hover:border-indigo-500/50 rounded-2xl p-3 cursor-pointer transition-all duration-150 flex flex-col justify-between group active:scale-95"
                    >
                      <div>
                        <div className="w-full h-24 rounded-xl overflow-hidden bg-slate-900 mb-2 border border-slate-800/50">
                          <img
                            src={prod.image}
                            alt={prod.nameFa}
                        ...
                        <div className="flex items-center justify-between text-[10px] text-slate-400 mt-1 font-mono">
                          ...
                          <span className="text-indigo-400">موجودی: {formatNumberFa(prod.stockCartons)}کارتن</span>
                        </div>
                      </div>
                      ...
                      <div className="mt-2 pt-2 border-t border-slate-800 flex items-center justify-between">
                        ...
                        <button className="p-1.5 rounded-lg bg-indigo-600/20 text-indigo-400 group-hover:bg-indigo-600 group-hover:text-white transition-colors">
            ...
            <div className="lg:col-span-5 space-y-4">
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 shadow-2xl flex flex-col justify-between min-h-[640px]">
                ...
                <div>
                  <div className="flex items-center justify-between pb-3 border-b border-slate-800 mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-sm font-black text-white">فاکتور جاری صندوق فروش</h3>
                    </div>
                    {posCart.length > 0 && (
                      <button
                        onClick={() => setPosCart([])}
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-3 bg-slate-950 p-2.5 rounded-2xl border border-slate-800">
                    <div>
                      <label className="block text-[10px] text-slate-400 font-bold mb-1">نام خریدار</label>
                      <input
                        type="text"
                        value={customerName}
                        onChange={(e) => setCustomerName(e.target.value)}
                        className="w-full bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1.5 text-xs text-white focus:outline-none focus:border-indigo-500 font-medium"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] text-slate-400 font-bold mb-1">همراه (جهت پیامک)</label>
                      <input
                        type="tel"
                        dir="ltr"
                        value={customerPhone}
                    ...
                    {posCart.length === 0 ? (
                      ...
                      posCart.map((item, idx) => (
                        <div
                          ...
                          className="bg-slate-950 border border-slate-800 rounded-2xl p-3 flex flex-col gap-2"
                        >
                            ...
                            <div className="flex items-center gap-1 bg-slate-900 p-0.5 rounded-lg border border-slate-800 text-[10px] font-bold">
                              {!item.product.isBoxOnly && (
                                <button
                                  ...
                                  className={`px-2 py-1 rounded ${item.unit === 'carton' ? 'bg-indigo-600 text-white' : 'text-slate-400'}`}
                                >
                                  کارتن
                                </button>
                              )}
                              <button
                                ...
                                className={`px-2 py-1 rounded ${item.unit === 'box' ? 'bg-indigo-600 text-white' : 'text-slate-400'}`}
                              >
                                باکس
                              </button>
                              <button
                                ...
                                className={`px-2 py-1 rounded ${item.unit === 'pack' ? 'bg-indigo-600 text-white' : 'text-slate-400'}`}
                              >
                                پاکت
                              </button>
                            </div>
                ...
                <div className="pt-4 border-t border-slate-800 space-y-3">
                  ...
                  {/* Payment Method Tabs */}
                  <div>
                    <label className="block text-[11px] font-bold text-slate-400 mb-1.5">روش پرداخت و تسویه:</label>
                    ...
                    <div className="flex items-center justify-between text-slate-400">
                      ...
                      <div className="flex items-center gap-1">
                        <input
                          ...
                          placeholder="۰"
                          className="w-24 bg-slate-900 border border-slate-800 rounded px-2 py-0.5 text-left font-mono text-xs text-emerald-400 focus:outline-none"
                        />
                        <span className="text-[10px]">تومان</span>
                      </div>
                    </div>

                    <div className="flex justify-between text-sm font-black text-indigo-300 pt-2 border-t border-slate-800">
                      <span>مبلغ نهایی دریافتی:</span>
                      <span className="text-base font-mono text-emerald-400">{formatToman(posFinalTotal)}</span>
                    </div>
                  </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div>
                  <h2 className="text-lg font-black text-white">مدیریت انبار مرکزی و کاردکس کالا</h2>
                  <p className="text-xs text-slate-400 mt-1">کنترل لحظه‌ای موجودی کارتن، باکس و ارزش ریالی دارایی‌های انبار سوین</p>
                </div>

                <div className="flex items-center gap-2">
                  <span className="text-xs font-black bg-indigo-500/10 text-indigo-400 border border-indigo-500/30 px-3 py-1.5 rounded-xl">
                    ...
                    {productsList.map((prod) => {
                      ...
                      return (
                        <tr key={prod.id} className="hover:bg-slate-800/50 transition-colors">
                          ...
                          <td className="p-3">
                            <strong className="text-white text-xs">{prod.nameFa}</strong>
                            <div className="text-[10px] text-slate-400">{prod.brand} • {prod.origin}</div>
                          </td>
                          ...
                          <td className="p-3 text-center">
                            {isLow ? (
                              <span className="bg-amber-500/10 text-amber-400 border border-amber-500/30 text-[10px] font-bold px-2 py-0.5 rounded-md flex items-center justify-center gap-1">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl">
              <h3 className="text-sm font-black text-white mb-4">گزارش کاردکس و گردش کالا در انبار</h3>
              <div className="space-y-2 max-h-[300px] overflow-y-auto">
                {stockLogs.length === 0 ? (
                  <p className="text-xs text-slate-500 text-center py-6">هنوز هیچ لاگ ورود یا خروج باری ثبت نشده است.</p>
                ) : (
                  stockLogs.map((log) => (
                    <div key={log.id} className="bg-slate-950 border border-slate-800 p-3 rounded-2xl flex items-center justify-between text-xs">
                      <div>
                        <span className="font-bold text-white">{log.productName}</span>
                        <div className="text-[10px] text-slate-400 mt-0.5">{log.date} • {log.note}</div>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className={`font-mono font-black ${log.deltaCartons > 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                          {log.deltaCartons > 0 ? `+${log.deltaCartons}` : log.deltaCartons} کارتن
                        </span>
                        <span className="text-[10px] bg-slate-900 px-2 py-1 rounded text-slate-400 font-mono">
        ...
        {activeSubTab === 'ledger' && (
          <div className="space-y-6">
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div>
                  <h2 className="text-lg font-black text-white">دفتر فاکتورهای فروش و تراکنش‌های صندوق</h2>
                  <p className="text-xs text-slate-400 mt-1">مشاهده فیش‌های صادر شده، چاپ مجدد و ریز اقلام مشتریان حضوری</p>
                </div>
              </div>
              ...
              {receiptsList.length === 0 ? (
                ...
                <div className="space-y-3">
                  {receiptsList.map((rcpt) => (
                    <div
                      key={rcpt.id}
                      className="bg-slate-950 border border-slate-800 rounded-2xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:border-indigo-500/40 transition-colors"
                    >
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-black text-white font-mono">{rcpt.receiptNumber}</span>
                        ...
                        <div className="text-xs text-slate-400 mt-1">
                          خریدار: <strong className="text-slate-200">{rcpt.customerName}</strong> • زمان ثبت: <span className="font-mono">{rcpt.createdAt}</span>
                        </div>
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-left">
                          <p className="text-[10px] text-slate-400 font-medium">مبلغ کل فاکتور</p>
                          <p className="text-sm font-black text-emerald-400 font-mono">{formatToman(rcpt.finalTotal)}</p>
                        </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-xs flex items-center justify-center p-4" dir="rtl">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-md w-full p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-black text-white">ورود یا اصلاح بار انبار</h3>
              <button onClick={() => setSelectedProductForAdjustment(null)} className="text-slate-400 hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>

            <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800 mb-4 flex items-center gap-3">
              ...
              <div>
                <h4 className="text-xs font-bold text-white">{selectedProductForAdjustment.nameFa}</h4>
                <p className="text-[11px] text-indigo-400 font-mono mt-0.5">موجودی فعلی: {selectedProductForAdjustment.stockCartons} کارتن</p>
              </div>
            </div>

            <div className="space-y-3">
              <div>
                <label className="block text-xs font-bold text-slate-300 mb-1">نوع عملیات انبارداری:</label>
                <select
                  value={adjustType}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-300 mb-1">تعداد کارتن:</label>
                <input
                  type="number"
                  min="1"
              ...
              <div>
                <label className="block text-xs font-bold text-slate-300 mb-1">توضیحات و شماره بارنامه:</label>
                <input
                  type="text"
                  value={adjustNote}
      ...
      {activeReceiptToPrint && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto" dir="rtl">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-sm w-full p-6 shadow-2xl">
            ...
            <div id="thermal-receipt" className="bg-white text-slate-900 p-5 rounded-2xl shadow-inner font-mono text-xs border border-slate-200">
              
              <div className="text-center border-b border-dashed border-slate-300 pb-3 mb-3">
                <div className="font-black text-sm text-slate-900 mb-1">مرکز پخش دخانیات سوین</div>
                <div className="text-[10px] text-slate-600">فاکتور رسمی فروش و رسید مشتری</div>
                <div className="text-[9px] text-slate-500 mt-1">تلفن پشتیبانی: ۰۹۱۲۰۷۵۹۴۱۹</div>
              </div>

              <div className="space-y-1 text-[10px] border-b border-dashed border-slate-300 pb-2 mb-2">
                <div className="flex justify-between">
                  <span>شماره فیش:</span>
                  <span className="font-bold">{activeReceiptToPrint.receiptNumber}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  <span>خریدار:</span>
                  <span>{activeReceiptToPrint.customerName}</span>
                </div>
              </div>
                  ...
                  <tbody>
                    {activeReceiptToPrint.items.map((it, idx) => (
                      <tr key={idx} className="border-b border-slate-100">
                        <td className="py-1 leading-tight">{it.product.nameFa}</td>
                        <td className="py-1 text-center font-bold">{it.quantity} {it.unit === 'carton' ? 'کارتن' : 'باکس'}</td>
                        <td className="py-1 text-left font-bold">{formatToman(it.totalPrice)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="text-center pt-3 mt-3 border-t border-dashed border-slate-300 text-[9px] text-slate-500">
                با سپاس از خرید شما • اصالت کلیه کالاها تضمین شده است
              </div>
            ...
            <div className="mt-4 flex items-center gap-2">
              ...
              <button
                ...
                className="py-3 px-4 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl text-xs font-bold"
              >
                بستن
              </button>
            </div>
