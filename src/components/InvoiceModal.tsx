...
export const InvoiceModal: React.FC<InvoiceModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 print:p-0 print:bg-white print:static animate-in fade-in duration-200">
      <div 
        className="bg-white border border-slate-200 rounded-3xl w-full max-w-4xl p-6 sm:p-8 shadow-2xl relative text-slate-900 print:border-none print:shadow-none print:w-full print:max-w-none transition-colors my-auto max-h-[92vh] flex flex-col overflow-hidden print:max-h-none print:overflow-visible"
        id="printable-invoice"
      >
        {/* Print Action Bar (Hidden on print) */}
        <div className="flex items-center justify-between pb-4 mb-6 border-b border-slate-200 print:hidden flex-wrap gap-3 shrink-0">
        ...
        <div className="flex-1 flex flex-col min-h-0 overflow-hidden space-y-4 print:overflow-visible print:space-y-4 print:h-auto">
          
          {/* Top Header of Invoice */}
          <div className="border-b-2 border-slate-900 pb-4 space-y-3 shrink-0">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50 p-3.5 rounded-2xl border border-slate-200 text-xs shrink-0">
          ...
          {/* Invoice Items Table */}
          <div className="flex-1 min-h-0 overflow-y-auto overflow-x-auto print:max-h-none print:overflow-visible rounded-2xl border border-slate-200 custom-scrollbar">
            <table className="w-full text-right text-xs">
              <thead className="bg-slate-100 text-slate-700 border-b border-slate-200 font-black text-[11px] sticky top-0 z-10 shadow-xs">
                <tr>
                  <th className="p-2.5 text-center w-10 bg-slate-100">ردیف</th>
                  <th className="p-2.5 bg-slate-100">شرح کالا و برند</th>
                  <th className="p-2.5 text-center bg-slate-100">واحد</th>
                  <th className="p-2.5 text-center bg-slate-100">تعداد</th>
                  <th className="p-2.5 text-left bg-slate-100">نرخ واحد (تومان)</th>
                  <th className="p-2.5 text-left bg-slate-100">تخفیف تیراژ</th>
                  <th className="p-2.5 text-left bg-slate-100">مبلغ کل (تومان)</th>
                </tr>
              </thead>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start bg-slate-50 p-4 rounded-2xl border border-slate-200 gap-4 text-xs shrink-0">
            <div className="space-y-2 text-slate-600 flex-1">
              ...
              <div className="border-t border-slate-200 pt-1.5 space-y-0.5 text-[11px]">
                <div className="font-black text-slate-800 mb-0.5">شماره حساب‌های مجاز جهت واریز وجه:</div>
            ...
            <div className="space-y-1.5 text-left w-full sm:w-72">
          ...
          <div className="grid grid-cols-2 gap-4 pt-2 text-center text-xs text-slate-500 shrink-0">
            <div className="border border-dashed border-slate-300 rounded-2xl p-3 h-20 flex flex-col justify-between">
