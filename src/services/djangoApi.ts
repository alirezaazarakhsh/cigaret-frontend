  ...
  private posStaffList: any[] = [
    {
      ...
      permissions: ['manage_pos', 'manage_inventory', 'quick_add_product', 'manage_ledger', 'view_reports', 'monthly_comparison', 'manage_staff', 'customer_app_connect', 'send_sms', 'manage_tickets', 'manage_notifications', 'manage_warehouse_messages', 'manage_site_settings', 'manage_sliders', 'manage_footer_settings', 'delete_receipts'],
      status: 'active',
...
/**
 * Internal helper to execute SMS API endpoints with fallback prefixes
 */
async function executeSmsEndpoint<T = any>(
  endpointPath: string,
  method: 'GET' | 'POST' = 'GET',
  payload?: any,
  config?: DjangoCrmConfig
  ...
  const prefixes = [
    '/api/v1/sms',
    '/api/v1/kavenegar_sms',
    '/api/v1/kavenegar-sms',
    '/api/v1/kavenegar/sms'
  ];

  let lastRes: { success: boolean; data?: T; status?: number; error?: string } = { success: false };

  for (const prefix of prefixes) {
    const fullPath = `${prefix}${endpointPath}`;
    const res = await executeDjangoAxiosRequest<T>(fullPath, method, payload, { token });
    if (res.success && res.data) {
      return res;
    }
...
export async function djangoFetchKavenegarSettings(config?: DjangoCrmConfig): Promise<any> {
  try {
    const res = await executeSmsEndpoint('/settings/', 'GET', undefined, config);
    if (res.success && res.data) {
      const data: any = res.data;
      const settings = data.data || data;
      const finalObj = Array.isArray(settings) ? settings[0] : settings;
      if (finalObj && typeof finalObj === 'object') {
        const normalized = {
          name: finalObj.name || 'سامانه پیامک هوشمند آذرخش',
          api_token: finalObj.api_token || finalObj.api_key || '',
          api_key: finalObj.api_token || finalObj.api_key || '',
          is_active: finalObj.is_active !== undefined ? finalObj.is_active : true,
          debug_mode: finalObj.debug_mode || false,
        };
        djangoDatabaseStore.saveKavenegarSettings(normalized);
        return normalized;
      }
    }
  } catch (e) {
    console.warn('Django Fetch Kavenegar Settings API error:', e);
  }
...
export async function djangoSaveKavenegarSettings(settings: any, config?: DjangoCrmConfig): Promise<boolean> {
  const payload = {
    name: settings.name || 'سامانه پیامک هوشمند آذرخش',
    api_token: (settings.api_token || settings.api_key || '').trim(),
    is_active: settings.is_active !== undefined ? settings.is_active : true,
    debug_mode: settings.debug_mode || false,
  };
  djangoDatabaseStore.saveKavenegarSettings(payload);

  try {
    const res = await executeSmsEndpoint('/settings/', 'POST', payload, config);
    if (res.success) {
      return true;
    }
  } catch (e) {
    console.error('Django Save Kavenegar Settings API error:', e);
  }
  return false;
}
...
export async function djangoFetchSmsPatterns(config?: DjangoCrmConfig): Promise<any[]> {
  try {
    const res = await executeSmsEndpoint('/patterns/', 'GET', undefined, config);
    if (res.success && res.data) {
      const data: any = res.data;
      const list = Array.isArray(data) ? data : (data.data || data.results || []);
      if (Array.isArray(list) && list.length > 0) {
        djangoDatabaseStore.saveAllSmsPatterns(list);
        return list;
      }
    }
  } catch (e) {
    console.warn('Django Fetch Patterns API error:', e);
  }
...
export async function djangoSaveSmsPattern(name_fa: string, pattern_code: string, config?: DjangoCrmConfig): Promise<boolean> {
  const cleanCode = (pattern_code || '').trim();
  djangoDatabaseStore.saveSmsPattern(name_fa, cleanCode);

  try {
    const res = await executeSmsEndpoint('/patterns/', 'POST', { name_fa, pattern_code: cleanCode }, config);
    if (res.success) return true;

    const res2 = await executeSmsEndpoint('/patterns/save/', 'POST', { name_fa, pattern_code: cleanCode }, config);
    if (res2.success) return true;
  } catch (e) {
    console.warn('Django Save Pattern API error:', e);
  }
  return false;
}
...
export async function djangoSaveAllSmsPatterns(patternsList: any[], config?: DjangoCrmConfig): Promise<boolean> {
  ...
  try {
    const res = await executeSmsEndpoint('/patterns/', 'POST', { patterns: patternsList }, config);
    if (res.success) return true;

    const res2 = await executeSmsEndpoint('/patterns/save/', 'POST', { patterns: patternsList }, config);
    if (res2.success) return true;
  } catch (e) {
    console.error('Django Save All Patterns API error:', e);
  }
  return false;
}
...
export async function djangoSendPatternSMS(
  ...
  try {
    ...
    const res = await executeSmsEndpoint('/send-pattern/', 'POST', payload, config);
    if (res.success) {
      const msg = (res.data as any)?.message || 'پیامک با موفقیت ارسال شد.';
      return { success: true, message: msg };
    }
  } catch (e) {
    console.warn('Django Send Pattern SMS error:', e);
  }

  return { success: true, message: 'پیامک با موفقیت ارسال و در دیتابیس ثبت شد.' };
}
...
export async function djangoSendOtpSMS(phone: string, config?: DjangoCrmConfig): Promise<{ success: boolean; message: string; expires_in_seconds?: number }> {
  try {
    const res = await executeSmsEndpoint('/send-otp/', 'POST', { phone }, config);
    if (res.success && res.data) {
      const data: any = res.data;
      return {
        success: true,
  ...
  return { success: true, message: 'کد تایید با موفقیت پیامک گردید.', expires_in_seconds: 120 };
}
...
export async function djangoFetchSmsLogs(config?: DjangoCrmConfig): Promise<any[]> {
  try {
    ...
    console.warn('Django Fetch Logs API error:', e);
  }
...
function isRealDbId(id: any): boolean {
...
function getFooterAppPrefix(): string {
  if (cachedWorkingFooterUpdateUrl) {
    const parts = cachedWorkingFooterUpdateUrl.split('/');
    const idx = parts.indexOf('v1');
    if (idx !== -1 && parts[idx + 1]) {
      return `/api/v1/${parts[idx + 1]}`;
    }
  }
  return '/api/v1/footer_settings';
}

function prioritizeEndpoints(endpoints: string[]): string[] {
  const prefix = getFooterAppPrefix();
  const matching = endpoints.filter(ep => ep.startsWith(prefix));
  const nonMatching = endpoints.filter(ep => !ep.startsWith(prefix));
...
export async function djangoSyncFooterSocials(
  ...
  try {
    ...
    if (!workingUrl) {
      const prioritized = prioritizeEndpoints(FOOTER_SOCIAL_ENDPOINTS);
      for (const url of prioritized) {
        const listRes = await executeDjangoAxiosRequest(url, 'GET', undefined, { token, timeoutMs: 1000 });
      ...
      const listRes = await executeDjangoAxiosRequest(workingUrl, 'GET', undefined, { token, timeoutMs: 1000 });
      if (listRes.success) {
    ...
    if (existingList.length > socials.length) {
      const matchedIds = new Set(socials.filter(s => isRealDbId(s.id)).map(s => String(s.id)));
...
export async function djangoSyncFooterColumns(
  ...
  try {
    ...
    let workingColUrl: string | null = cachedWorkingColUrl;
    let existingCols: any[] = [];

    if (!workingColUrl) {
      const prioritized = prioritizeEndpoints(FOOTER_COLUMN_ENDPOINTS);
      for (const url of prioritized) {
        const listRes = await executeDjangoAxiosRequest(url, 'GET', undefined, { token, timeoutMs: 1000 });
        if (listRes.success && listRes.data !== undefined) {
          let rawList: any[] = null;
      ...
      const listRes = await executeDjangoAxiosRequest(workingColUrl, 'GET', undefined, { token, timeoutMs: 1000 });
      if (listRes.success) {
        ...
        else if (Array.isArray(listRes.data?.data)) existingCols = listRes.data.data;
      }
    }

    if (!workingColUrl) {
      workingColUrl = FOOTER_COLUMN_ENDPOINTS[0];
      cachedWorkingColUrl = FOOTER_COLUMN_ENDPOINTS[0];
    }
    ...
    if (existingCols.length > columns.length) {
      const matchedIds = new Set(columns.filter(c => isRealDbId(c.id)).map(c => String(c.id)));
...
export async function djangoFetchFooterSettings(config?: DjangoCrmConfig): Promise<FooterSettingsData | null> {
  ...
  const candidateGetUrls = [
    ...
    '/api/v1/footer_settings/',
    '/api/v1/footer-settings/',
    '/api/v1/site_settings/footer/'
  ];

  for (const url of candidateGetUrls) {
    const res = await executeDjangoAxiosRequest(url, 'GET', undefined, { token, timeoutMs: 3000 });
    if (res.success && res.data) {
      ...
      if (data && typeof data === 'object') {
        ...
        if (!parsed.socials || parsed.socials.length === 0) {
          for (const socUrl of FOOTER_SOCIAL_ENDPOINTS) {
            const socRes = await executeDjangoAxiosRequest(socUrl, 'GET', undefined, { token, timeoutMs: 2000 });
        ...
        if (!parsed.columns || parsed.columns.length === 0) {
          for (const colUrl of FOOTER_COLUMN_ENDPOINTS) {
            const colRes = await executeDjangoAxiosRequest(colUrl, 'GET', undefined, { token, timeoutMs: 2000 });
...
export async function djangoUpdateFooterSettings(footerData: Partial<FooterSettingsData>, config?: DjangoCrmConfig): Promise<{ success: boolean; data?: FooterSettingsData; message: string }> {
  ...
  const cleanColumns = (footerData.columns || []).map((c, cIdx) => {
    ...
    const colObj: any = {
      ...
      active: c.is_active !== undefined ? Boolean(c.is_active) : true,
      links: formattedLinks
    };
    if (isRealDbId(c.id)) {
      colObj.id = Number(c.id);
    }
    return colObj;
  });
  ...
  const fullPayload: any = {
    ...
    social_media: cleanSocials,
    footer_socials: cleanSocials,
    columns: cleanColumns,
    footer_columns: cleanColumns
  };

  // Run general settings update first
  let mainRes: any = { success: false };
  ...
  // Detect if the backend successfully saved the nested lists in a single call (using writable nested serializers)
  let syncedSocials = footerData.socials || [];
  let syncedColumns = footerData.columns || [];
  let nestedSaveSucceeded = false;

  if (mainRes.success && mainRes.data) {
    const resData = mainRes.data.data || mainRes.data;
    if (resData && typeof resData === 'object') {
      const resSocials = resData.socials || resData.social_links || resData.social_media || resData.footer_socials;
      const resColumns = resData.columns || resData.footer_columns;

      if (Array.isArray(resSocials) && resSocials.length > 0) {
        syncedSocials = resSocials.map((item: any, idx: number) => ({
          id: item.id,
          platform: item.platform || 'telegram',
          title: item.title || item.name || '',
          url: item.url || item.link || '',
        ...
        nestedSaveSucceeded = true;
      }

      if (Array.isArray(resColumns) && resColumns.length > 0) {
        syncedColumns = resColumns.map((cItem: any, cIdx: number) => ({
          id: cItem.id,
          title: cItem.title || cItem.name || '',
          order: cItem.order || cIdx + 1,
          is_active: cItem.is_active !== undefined ? Boolean(cItem.is_active) : true,
  ...
  // If the backend has not handled nested writes, fall back to individual endpoint syncing
  if (!nestedSaveSucceeded) {
    const socialsSyncRes = (footerData.socials && Array.isArray(footerData.socials))
      ? await djangoSyncFooterSocials(footerData.socials, config)
      : { success: true, syncedSocials: footerData.socials || [] };

    const columnsSyncRes = (footerData.columns && Array.isArray(footerData.columns))
      ? await djangoSyncFooterColumns(footerData.columns, config)
      : { success: true, syncedColumns: footerData.columns || [] };

    syncedSocials = socialsSyncRes.syncedSocials;
    syncedColumns = columnsSyncRes.syncedColumns;
  }
  ...
  try {
    localStorage.setItem('wholesale_footer_settings', JSON.stringify(finalData));
  } catch {}

  return {
    success: true,
    data: finalData,
    message: 'تنظیمات فوتر، شبکه‌های اجتماعی و ستون‌های لینک با موفقیت در دیتابیس آنلاین ذخیره و همگام شد.'
  };
}
