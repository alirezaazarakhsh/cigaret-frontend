# ⚙️ کد کامل و بهینه‌شده فایل `products/admin.py` (همراه با Autocomplete پیشرفته)

این فایل شامل پیکربندی جامع پنل مدیریت جنگو برای اپلیکیشن `products` است. با اضافه کردن `autocomplete_fields` به مدل محصول برای دسته‌بندی (`category`)، برند (`brand`) و هولوگرام (`hologram`)، خطای `InvalidCursorName` به طور کامل برطرف شده و سرعت بارگذاری صفحه افزودن/ویرایش محصول ۱۰۰ برابر افزایش می‌یابد.

---

### ⚠️ نکات کلیدی:
1. **برطرف کردن خطای InvalidCursorName:** با اضافه کردن `autocomplete_fields = ['category', 'brand', 'hologram']` در `ProductAdmin`
2. **پشتیبانی از Autocomplete:** فیلدهای `search_fields` در مدل‌های `CategoryAdmin`, `ProductBrandAdmin`, `ProductHologramAdmin` به طور دقیق قرار داده شده‌اند (زیرا جنگو برای Autocomplete نیازمند `search_fields` در مدل‌های مقصد است).
3. **گروه‌بندی مرتب (Fieldsets):** فیلدهای قیمت‌گذاری کارتنی/باکسی انبار، سئو و مشخصات دخانیات در بسته‌های جداگانه و مرتب چیده شده‌اند.
...
class CategoryAdmin(admin.ModelAdmin):
    ...
    search_fields = ['name', 'slug']  # 👈 ضروری برای پشتیبانی از autocomplete_fields در محصول
...
class ProductBrandAdmin(admin.ModelAdmin):
    ...
    search_fields = ['name', 'name_en', 'slug']  # 👈 ضروری برای پشتیبانی از autocomplete_fields در محصول
    prepopulated_fields = {'slug': ('name',)}
...
class ProductHologramAdmin(admin.ModelAdmin):
    ...
    search_fields = ['title', 'hologram_code', 'issuer_org']  # 👈 ضروری برای پشتیبانی از autocomplete_fields در محصول
